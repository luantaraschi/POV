...
export function chooseCopyEditAgent({
  ...
  // SECURITY: default to 'off' so the permission/sandbox-bypassing copy-edit
  // agent (claude --permission-mode bypassPermissions / codex
  // --dangerously-bypass-approvals-and-sandbox) never spawns automatically.
  // Set IMPECCABLE_LIVE_COPY_AGENT=auto|codex|claude|chat|mock to opt back in.
  const mode = (env.IMPECCABLE_LIVE_COPY_AGENT || 'off').trim().toLowerCase();
