...
// SECURITY: a request Origin is "loopback" only when its host is localhost,
// 127.0.0.1, ::1, or a *.localhost name. Used to reject cross-origin requests
// from foreign websites so they cannot scrape the session token or drive
// state-changing routes against the local live server.
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    // SECURITY: only same-machine (loopback) browser origins may talk to this
    // server. A foreign website must not be able to read token-bearing
    // responses (e.g. /live.js) or drive write routes cross-origin. Node-side
    // callers (live-poll/status/etc.) send no Origin header and are unaffected.
